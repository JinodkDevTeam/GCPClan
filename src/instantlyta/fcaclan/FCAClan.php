...
use JsonException;
use LDX\iProtector\Main as IProtector;
...
class FCAClan extends PluginBase implements Listener{
	...
	protected array $clans = [];
	protected array $invitePending = [];
	protected array $inviteSendConfirm = [];
	protected array $kickPending = [];
	protected array $quitPending = [];
	protected array $welcomePending = [];
	protected array $clanPromotePending = [];
	protected array $clanDeletePending = [];

	public array $topClan = [];

	private static ?FCAClan $instance = null;
	private ?IProtector $iprotect;
	...
	public function updateNametag(ClanChangeEvent $event) : void{ // PureChat API
		...
		$levelName = $isMultiWorldSupportEnabled ? $p->getWorld()->getDisplayName() : null;

		$p->setNameTag($pureChat->getNameTag($p, $levelName));
	}


	public function onDamagee(EntityDamageEvent $event) : void{
		if(!($event->isCancelled() or !$this->iprotect->canGetHurt($event->getEntity()))){
			if($event instanceof EntityDamageByEntityEvent){
				$damager = $event->getDamager();
				$entity = $event->getEntity();
				if(!($damager instanceof Player) || !($entity instanceof Player) || !$this->iprotect->canGetHurt($damager)){
					return;
				}
	...
	protected function paginateText(CommandSender $sender, $pageNumber, array $txt) : bool{
		$hdr = array_shift($txt);
			...
			case "clan":
				...
				switch($args[0]){
					...
					case "invite":
						...
						$player = $this->getServer()->getPlayerByPrefix($altArgs[0]);
						if($player === null || $player->getName() === $sender->getName()){ // TODO this. is. bad.
							...
							return true;
						}elseif($player->getName() === $altArgs[0] && $this->getProfile($player)->get("clan") !== false){
					...
					case "setrank":
						...
						if(isset($altArgs[1])){
							...
							if((string) ($altArgs[1]) !== (string) $altArgs[1]){
							...
							if($altArgs[1] >= $profile->get("rank") && $profile->get("rank") !== self::OWNER_RANK){
								...
								return true;
							}elseif($altArgs[1] === $targetProfile->get("rank")){
					...
					case "top":
						...
						for($i = 0; $i < 10; $i++){
							$mes .= "§r⨶§l§b Xếp Hạng§e " . ($i + 1) . " §f:§a " . ($this->topClan[$i] ?? "§cChưa cập nhật!") . "\n";
						}
	 ...
	 * @return Config
	 */
	public function getProfile(Player|string $player) : Config{
		...
		return new Config($dir . "$player.yml", Config::YAML, ["clan" => false, "rank" => 0]);
	}
	 ...
	 * @return bool|null
	 */
	public function canDo(Player|string $player, int $action) : ?bool{
	...
	public function actionToString(int $action) : ?string{
		return match ($action) {
			self::ACTION_INVITE => "invite",
			self::ACTION_KICK => "kick",
			self::ACTION_REQUEST_CONTROL => "requestcontrol",
			self::ACTION_SET_MOTD => "setmotd",
			self::ACTION_SET_RANK => "setrank",
			self::ACTION_LEVEL_UP => "levelup",
			default => null,
		};
	}
	 ...
	 * @return bool
	 */
	public function addPlayerToClan(Player|string $player, string $clanName) : bool{
		if($player instanceof Player){
			$player = $player->getName();
		}
		$profile = $this->getProfile($player);
		...
		if(($tmp = $this->getServer()->getPlayerExact($player)) !== null){
			$ev = new ClanChangeEvent($tmp, $clanName);
			$ev->call();
		}
	 ...
	 * @return bool
	 */
	public function removePlayerFromClan(Player|string $player) : bool{
		if($player instanceof Player){
			$player = $player->getName();
		}
		$profile = $this->getProfile($player);
	...
	public function haveClan($player) : bool{
	...
	public function getClanTag($player) : array|string{
		/** @var string $style */
		$style = $this->getSetting(self::SETTING_CLAN_TAG, $this->getClan($this->getClanName($player))["level"]);
		return str_replace("%s", $this->getClanName($player), $style);
	}
	...
	public function isClanFull(string $clanName) : bool{
	 ...
	 * @return null|string
	 */
	public function getPlayerInClan(string $needle, string $clanName, string $except = null) : ?string{
		$clan = $this->getClan($clanName);
		$found = null;
	 ...
	 * @return null|string
	 */
	public function getPlayerInRequestList(string $needle, string $clanName, string $except = null) : ?string{
		$clan = $this->getClan($clanName);
		$found = null;
		$needle = strtolower($needle);
		$delta = PHP_INT_MAX;
